...
async def node_analyze(state):
    ...
    await audit(state, AuditEntry(
        ...
        reason=a.confidence_reasoning, execution_time_ms=int((time.monotonic() - t0) * 1000)
    ))
    return {"analysis": a}
...
async def node_route(state):
    ...
    await audit(state, AuditEntry(
        ...
        reason=f"Routed based on confidence {c:.0%}", execution_time_ms=int((time.monotonic() - t0) * 1000)
    ))
...
async def node_human_approval(state):
    ...
    await audit(state, AuditEntry(
        agent_id=AGENT_ID, action="human_approval", confidence=a.confidence,
        risk_level=risk_level_for(a.confidence), decision="pending",
        reason="Waiting for human approval", execution_time_ms=int((time.monotonic() - t0) * 1000)
    ))
    ...
    await audit(state, AuditEntry(
        agent_id=AGENT_ID, action="human_approval", confidence=a.confidence,
        risk_level=risk_level_for(a.confidence), decision=choice or "pending", 
        reviewer_id=os.environ.get("GITHUB_USER", "unknown"),
        reason=resp.get("feedback") or f"Human choice: {choice}", 
        execution_time_ms=int((time.monotonic() - t0) * 1000)
    ))
...
async def node_commit(state):
    ...
    c = a.confidence if a else 0.0
    await audit(state, AuditEntry(
        agent_id=AGENT_ID, action="commit", confidence=c,
        risk_level=risk_level_for(c), decision="approve" if action.startswith("commit") else "reject",
        reason=f"Final action: {action}", execution_time_ms=int((time.monotonic() - t0) * 1000)
    ))
...
async def node_auto_approve(state):
    ...
    await audit(state, AuditEntry(
        ...
        reason="Auto-approved due to high confidence", execution_time_ms=int((time.monotonic() - t0) * 1000)
    ))
...
async def node_escalate(state):
    ...
    await audit(state, AuditEntry(
        agent_id=AGENT_ID, action="escalate", confidence=a.confidence,
        risk_level=risk_level_for(a.confidence), decision="pending",
        reason="Escalating to human", execution_time_ms=int((time.monotonic() - t0) * 1000)
    ))
    ...
    await audit(state, AuditEntry(
        agent_id=AGENT_ID, action="escalate", confidence=a.confidence,
        risk_level=risk_level_for(a.confidence), decision="escalate", 
        reviewer_id=os.environ.get("GITHUB_USER", "unknown"),
        reason="Received answers from human", execution_time_ms=int((time.monotonic() - t0) * 1000)
    ))
...
async def node_synthesize(state):
    ...
    await audit(state, AuditEntry(
        ...
        reason=refined.confidence_reasoning, execution_time_ms=int((time.monotonic() - t0) * 1000)
    ))
