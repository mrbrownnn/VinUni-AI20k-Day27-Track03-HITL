    ...
    with console.status("[dim]LLM reviewing the diff...[/dim]"):
        analysis = llm.invoke([
            {"role": "system", "content": (
                ...
                "If confidence is less than 60%, populate escalation_questions "
                "with 2-4 specific, context-rich questions referencing the diff."
            )},
...
def node_synthesize(state: ReviewState) -> dict:
    ...
    with console.status("[dim]LLM refining the review...[/dim]"):
        analysis = llm.invoke([
            {"role": "system", "content": "Refine the review based on the reviewer's answers. Output the updated structured PRAnalysis."},
            {"role": "user", "content": f"Title: {state['pr_title']}\nOriginal Diff:\n{state['pr_diff']}\n\nReviewer Q&A:\n{qa_text}"},
        ])
    return {"analysis": analysis}
